...
Company_demos <- read_csv("Your file Here 1")

#Reading in Labor Force Participation Rates and Census Numbers
Gender_LFPR <- read_csv("Labor force participation by Gender and state.csv")
...
Gender_labor_adjusted <- left_join(Gender_state, Gender_LFPR, by = c("Geography"="State"))
Gender_labor_adjusted1.5 <- mutate(Gender_labor_adjusted, Male2 = Male*Male_LFPR, Female2= Female*Female_LFPR)
Gender_labor_adjusted2 <- mutate(Gender_labor_adjusted1.5, Females_in_workforce_percent = Female2 / (Male2+Female2), Males_in_workforce_percent = Male2 / (Male2+Female2), total_workforce = Male2 + Female2 )
